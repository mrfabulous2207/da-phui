...
/* Chi dao ca nhan: thieu khai bao trong EXTRA_RULE thi num hien ra trang tron,
   khong noi duoc la no nam san trong vai hay da nhau voi vai. */
EXTRAS.forEach(x => {
  ok(!!EXTRA_RULE[x[0]], "chi dao ca nhan " + x[0] + " co khai bao trong EXTRA_RULE");
  const r = EXTRA_RULE[x[0]] || { part: [], conflict: [] };
  eq(r.part.filter(id => r.conflict.indexOf(id) >= 0).length, 0,
     x[0] + ": mot vai khong the vua nam san vua da nhau");
});
eq(new Set(EXTRAS.map(x => x[0])).size, EXTRAS.length, "khong chi dao ca nhan nao trung ma");
eq(EXTRAS.length, 8, "tam chi dao ca nhan");

/* Hai chi dao da nhau tren CUNG mot nguoi -- truoc day tich ca hai van im lang. */
EXTRA_PAIR.forEach(pr => {
  ok(extraFights(pr[0], [pr[1]]), pr[0] + " da nhau voi " + pr[1]);
  ok(extraFights(pr[1], [pr[0]]), "va nguoc lai");
  ok(EXTRAS.some(x => x[0] === pr[0]) && EXTRAS.some(x => x[0] === pr[1]),
     "cap " + pr.join("/") + " phai tro toi chi dao co that");
});
eq(extraFights("cut", []), false, "chua bat gi thi khong da nhau");
eq(extraFights("cut", ["shoot"]), false, "cat vao trong va sut xa di duoc voi nhau");
eq(extraFights("khong-co-that", ["wide"]), false, "ma la thi khong nem loi");
eq(extraFights("cut", null), false, "danh sach null khong nem loi");
